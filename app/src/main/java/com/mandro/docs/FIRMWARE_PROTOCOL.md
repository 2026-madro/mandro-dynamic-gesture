...
> ⚠️ **구방식 (서버 기반) 문서 — 로컬 전환 후에는 `docs/LOCAL_MIGRATION.md`의 방식(A안: LittleFS)을 따른다.**
> 이 절은 서버가 학습마다 펌웨어 전체를 재컴파일하던 기존 흐름을 그대로 기록한 것이며,
> `UsbRepositoryImpl.kt`도 한때 이 방식의 변형(MDRO 매직넘버 + model.tflite/scaler.bin)으로
> 구현돼 있었으나 지금은 `LOCAL_MIGRATION.md` 프로토콜(`0xDEADBEEF` + 단일 weights.bin)로 교체됨.
