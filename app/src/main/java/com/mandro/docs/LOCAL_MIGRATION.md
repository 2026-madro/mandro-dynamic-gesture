...
| | 기존 방식 | A 방식 |
|---|---|---|
| 가중치 전달 | 펌웨어에 하드코딩 후 전체 `.bin` 플래시 | 별도 파일로 전송 → SPIFFS 저장 |
...
[저장]   ESP32 → SPIFFS에 weights.bin 저장 → 재부팅
[추론]   ESP32 부팅 시 SPIFFS에서 가중치 로드 → BLE로 추론 결과 전송
...
A 방식에서는 SPIFFS 파일에서 동적으로 읽도록 변경한다.
...
| **가중치 수신 모드** | 매직 넘버 수신 (BLE Write 또는 USB Serial) | 가중치 바이너리 수신 → SPIFFS 저장 → 재부팅 |
| **추론 모드** | SPIFFS에 가중치 존재 시 자동 활성 | BLE로 추론 결과 전송 (현재와 동일) |
...
// 가중치는 SPIFFS /weights.bin 에서 런타임에 로드
...
**`nn.h / nn.cpp`** — 생성자에서 SPIFFS 파일 로드
```cpp
// nn.h 변경
class NeuralNet {
public:
    NeuralNet();
    bool loadFromSPIFFS(const char* path = "/weights.bin");
...
```cpp
// nn.cpp — loadFromSPIFFS 구현
#include <SPIFFS.h>

bool NeuralNet::loadFromSPIFFS(const char* path) {
    File f = SPIFFS.open(path, "r");
    if (!f) return false;

    const size_t EXPECTED = 13062 * sizeof(float);
    if (f.size() != EXPECTED) { f.close(); return false; }

    // 레이어 순서: W0 b0 W1 b1 W2 b2
    // W0(132×64) b0(64) W1(64×64) b1(64) W2(64×6) b2(6)
...
**`exo_armband_hybrid.ino`** — SPIFFS 초기화 + 가중치 수신 모드 추가
```cpp
#include <SPIFFS.h>

#define WEIGHT_MAGIC  0xDEADBEEF
#define WEIGHTS_PATH  "/weights.bin"
#define WEIGHT_TOTAL  52248  // 13062 floats × 4 bytes

void setup() {
    // ... 기존 setup 코드 유지 ...

    SPIFFS.begin(true);  // true = 마운트 실패 시 포맷

    if (nn.loadFromSPIFFS(WEIGHTS_PATH)) {
...
#### SPIFFS 파티션
`partitions.csv`에 SPIFFS 파티션 최소 256KB 확보 필요 (가중치 51KB + 여유분).

#### 최초 플래시 절차 (1회만)
```bash
# PC에서 arduino-cli로 SPIFFS 지원 펌웨어 빌드 및 플래시
...
def serialize_weights(model) -> bytes:
    """sklearn MLPClassifier 가중치를 바이너리로 직렬화.
...
#### 전송 프로토콜
USB Serial 및 BLE 모두 동일한 패킷 포맷 사용.

```
[4 bytes] 매직 넘버: 0xDEADBEEF (little-endian)
[4 bytes] 총 바이트 수: 52248 (uint32 little-endian)
[N bytes] 가중치 바이너리 (float32, 레이어 순서대로)
[4 bytes] CRC32 체크섬
```

BLE 전송 시에는 MTU(244바이트) 단위로 청크 분할.
```
패킷 수: 52,248 / 200 ≈ 262개
...
```
1단계  펌웨어 — SPIFFS 가중치 수신/로드 구현
         └─ MODEL.h에서 가중치 배열 제거
         └─ nn.cpp에 loadFromSPIFFS() 구현
...
| SPIFFS (ESP32) | arduino-esp32 내장 | 가중치 파일 저장 |
